...
class Jobs_model extends CI_Model {
    ...
    public function getJobsByPage($page, $resultsPerPage = 50) {
        ...
        if($store == 0 || $store == null)
            $this->db->orderBy('id')->limit(($page * $resultsPerPage), $resultsPerPage)->select('jobs');
        else
            $this->db->where('store_id', $store)->orderBy('id')->limit(($page * $resultsPerPage), $resultsPerPage)->select('jobs');
        return $this->db->fetchAll();
    }
