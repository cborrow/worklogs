...
class iCalendarEvent {
	...
	public function setStartTime($time) {
		if(is_numeric($time))
			$this->dtStart = date("Ymd\This", $time);
		else
			$this->dtStart = $time;

		$this->dtStamp = $this->dtStart;
	}
	...
	public function toString() {
		if($this->organizer == null)
			throw new Exception("An origanizer is required.");
		...
		$output .= "CREATED:" . date("Ymd\This", time()) . "\r\n";;
		...
		if(count($parts) > 0)
			...
			$output .= $temp . "\r\n";
		$output .= "LAST-MODIFIED:" . date("Ymd\This", time()) . "\r\n";;
	...
	/**
	 * Returns a formmated trigger time for iCalendar VALARM:TRIGGER.
	 * 
	 * @access private
